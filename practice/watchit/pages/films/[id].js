...
export async function getServerSideProps(context) {
  ...
  let movieReviews;
  try {
    const movieRes = await instance.get(`/movies/${movieId}`);
    const reviewsRes = await instance.get(`/movie_reviews?movie_id=${movieId}`);
    movie = movieRes.data ?? [];
    movieReviews = reviewsRes.data.results ?? [];
  } catch (error) {
...
export default function Movie({ movie, movieReviews: initialMovieReviews }) {
  ...
  async function handleSubmit(e) {
    ...
    setMovieReviews((prevMovieReviews) => {
      return [nextMovieReview, ...prevMovieReviews];
    });
  }
